# 第 2 章：全局 Context Prompt — 系统提示词的构建机制
...
在 LLM 应用中，系统提示词决定了 AI 的身份、能力边界和行为规范。Claude Code 的系统提示词并非一成不变——它根据当前模式（普通模式 / Coordinator 模式 / Proactive 模式 / 自定义 Agent 模式）**动态构建**，并在每一轮 API 调用前组装。
...
优先级（从高到低）：
  ...
  0. Override        ← loop 模式等特殊场景，完全替换所有其他提示词
  1. Coordinator     ← 自主代理协调器模式（环境变量 CLAUDE_CODE_COORDINATOR_MODE=1）
  2. Agent           ← mainThreadAgentDefinition 定义的自定义代理
     ├── Proactive 模式：Agent 提示词 "追加" 到 Default 之后
     └── 普通模式：Agent 提示词 "替换" Default
  3. Custom          ← 用户通过 --system-prompt CLI 参数传入
  4. Default         ← Claude Code 内置的标准提示词
  + Append           ← 始终追加在末尾（Override 除外）
```
...
export function buildEffectiveSystemPrompt({
  ...
  // 优先级 2（Proactive 模式）：Agent 提示词追加到 Default 之后
...
除了系统提示词，Claude Code 还向每轮 API 调用注入动态上下文。附件消息类型为 `AttachmentMessage`，在 `normalizeMessagesForAPI()` 时转换为 API 格式（通常是 `<system-reminder>` 包裹的 user message）。
...
| `nested_memory` | 用户 @-mention 文件时，自动加载该文件所在目录的 CLAUDE.md 规则 |
...
## 2.9 Coordinator 模式系统提示词

Coordinator 模式是 Claude Code 的"自主代理编排"模式，此时系统提示词完全不同：
...
Claude API 支持 **Prompt Cache**（提示词缓存）。Claude Code 通过以下方式最大化缓存命中：

1. **系统提示词内容稳定**：尽量不在每次循环中改变系统提示词内容
2. **Delta 附件模式**：只在有变化时才追加新内容，避免整个消息列表发生变化
3. **Fork Subagent 字节级一致**：子代理创建时传递"已渲染"的系统提示词，而不是重新调用 `getSystemPrompt()`（可能因 GrowthBook 状态变化而产生不同结果）
...
export const FORK_AGENT = {
  // ...
  getSystemPrompt: () => '',  // 不使用，由 override 传递父代理的已渲染提示词
}
```
...
| 机制 | 作用 | 源码位置 |
|------|------|----------|
| `buildEffectiveSystemPrompt()` | 按优先级组合系统提示词 | [src/utils/systemPrompt.ts:41](../../src/utils/systemPrompt.ts#L41) |
...
| Coordinator | 自主代理编排模式 | `src/coordinator/coordinatorMode.ts` |
